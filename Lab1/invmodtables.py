...
def create_modulo_table(operation, p):
    table = [[0] * p for _ in range(p)]

    for i in range(p):
        for j in range(p):
            if operation == "add":
                table[i][j] = add_mod_p(i, j, p)
            elif operation == "multiply":
                table[i][j] = multiply_mod_p(i, j, p)
...
# Create modulo table for addition and multiplication
addition_table = create_modulo_table("add", p)
multiplication_table = create_modulo_table("multiply", p)

# Display addition modulo table
print("\nAddition Modulo Table:")
for row in addition_table:
    print(row)

# Display multiplication modulo table
print("\nMultiplication Modulo Table:")
for row in multiplication_table:
    print(row)

# Get input elements from the user
element1 = int(input("\nEnter the first element in GF(p): "))
...
# Display results
print(f"\nAddition: {element1} + {element2} = {result_add}")
